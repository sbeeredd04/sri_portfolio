...
function fadeInAndSlideInOnScroll() {
  ...
  const scrollVariable = Math.max(
    0,
    Math.min(1, (windowHeight - imageTop + 200) / windowHeight)
  );

  // Calculate the opacity and transform based on the variable
  const opacity = scrollVariable;
  const slideInAmount = 350 * (1 - scrollVariable); // 100 is the distance the image slides in

  // Apply the calculated styles using requestAnimationFrame
